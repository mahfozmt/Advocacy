...
// 2. Line-based replacement for Part C
...
for (let i = 0; i < lines.length; i++) {
    if (lines[i].includes('#### (গ) "Plaintiff Must Prove His Own Case"')) {
        partCStart = i;
    }
    if (lines[i].includes('#### (ঘ) মুনসেফ কোর্টের')) {
        partCEnd = i;
        partDStart = i;
    }
    if (lines[i].includes('#### (ঙ) Selective Entry')) {
        partDEnd = i;
    }
}

console.log(`Part C Start: ${partCStart}, End: ${partCEnd}`);
console.log(`Part D Start: ${partDStart}, End: ${partDEnd}`);

if (partCStart !== -1 && partCEnd !== -1) {
...
`;
    ...
    lines.splice(partCStart, partCEnd - partCStart, replacementPartC);
    
    // After splice, indices are shifted. Let's recalculate partDStart and partDEnd
    partDStart = -1;
    partDEnd = -1;
    for (let i = 0; i < lines.length; i++) {
        if (lines[i].includes('#### (ঘ) মুনসেফ কোর্টের')) {
    ...
    console.log(`Recalculated Part D Start: ${partDStart}, End: ${partDEnd}`);
}

if (partDStart !== -1 && partDEnd !== -1) {
...
`;
    ...
    lines.splice(partDStart, partDEnd - partDStart, replacementPartD);
}

let modifiedContent = lines.join('\n');

// 3. Inject the new "Third Part" and push Prayer to Fourth Part
// Direct string replace for the prayer part
...
// Fallback replacement if the above emoji format was different
if (!modifiedContent.includes('## চতুর্থ অংশ: প্রার্থনা')) {
    modifiedContent = modifiedContent.replace(
...
modifiedContent = modifiedContent.replace(targetTableEnd, replacementTableEnd);

// Restore CRLF for Windows compatibility
modifiedContent = modifiedContent.replace(/\n/g, '\r\n');

fs.writeFileSync(destPath, modifiedContent, 'utf8');
console.log("Done constructing v19 robustly!");
